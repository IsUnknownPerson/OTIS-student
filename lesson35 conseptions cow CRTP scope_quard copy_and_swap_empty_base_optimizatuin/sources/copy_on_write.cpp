...
namespace copy_on_write {
	...
	void example() {
		...
		std::cout << std::hex << "str pointer: " << reinterpret_cast<int *>(str->data()) << std::endl;

		// Do copy without copying
		const COW_string copy1{str};
		std::cout << "copy1 value: " << *copy1 << std::endl;
		std::cout << std::hex << "copy1 pointer: " << reinterpret_cast<const int *>(copy1->data()) << std::endl;

		const COW_string copy2{str};
		std::cout << "copy2 value: " << *copy2 << std::endl;
		std::cout << std::hex << "copy2 pointer: " << reinterpret_cast<const int *>(copy2->data()) << std::endl;

		// Do change
		COW_string copy3{copy2};
		*copy3 += " Changed!";
		std::cout << "copy3 value after change: " << *copy3 << std::endl;
		std::cout << std::hex << "copy3 pointer after change: " << reinterpret_cast<int *>(copy3->data()) << std::endl;
		std::cout << "copy1 value: " << *copy1 << std::endl;
		std::cout << "copy2 value: " << *copy2 << std::endl;
