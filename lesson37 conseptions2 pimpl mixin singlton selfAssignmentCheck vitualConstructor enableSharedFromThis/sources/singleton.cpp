...
public:
    ...
    void debug(const std::string &msg) { m_file << "[DEBUG] " << msg << std::endl; }
    void info(const std::string &msg) { m_file << "[INFO] " << msg << std::endl; }
    void error(const std::string &msg) { m_file << "[ERROR] " << msg << std::endl; }
...
public:
    ...
    {
        static Logger logger{path}; // в многопотоке сработает благодаря static
        return logger;
    }
