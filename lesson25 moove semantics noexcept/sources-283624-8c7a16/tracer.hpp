...
public:
    ...
    //Tracer(Tracer &&rhs) = delete; запрет перемещения
    Tracer(Tracer &&rhs) noexcept : m_name(std::move(rhs.m_name)) {  // NOLINT
        std::cout << "  Move ctor from \"" << m_name << "\" #" << m_id << std::endl;
    // в данном случае это необязательно, т.к. компилятор для
    // простого Tracer сможет сгенерить мув конструктор по умолчанию
    }
    ...
    Tracer & operator =(Tracer &&rhs) noexcept {  // NOLINT
        std::cout
